...
parser.add_argument('--input_dir', type=str, default='input', help="Input directory where where training dataset and meta data are saved")
parser.add_argument('--output_dir', type=str, default='output', help="Output directory where output such as logs are saved.")
...
token_to_id_path        = os.path.join(datadir, 'ptb', 'token2id.txt')
validation_file_path    = os.path.join(datadir,'ptb', 'valid.txt')
train_file_path         = os.path.join(datadir,'ptb', 'train.txt')
token_frequencies_file_path = os.path.join(datadir,'ptb', 'freq.txt')
...
def print_progress(samples_per_second, average_full_ce, total_samples, total_time):
    ...
    with open(os.path.join(outputdir,"log.txt"), "a+") as myfile:
...
def train_lm():
    ...
    for epoch_count in range(num_epochs):
        ...
        z.save(os.path.join(outputdir, model_filename))
