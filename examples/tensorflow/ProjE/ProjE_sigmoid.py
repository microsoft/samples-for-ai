...
class ProjE:
    ...
    def __init__(self, input_dir, embed_dim=100, combination_method='simple', dropout=0.5, neg_weight=0.5):
        ...
        with open(os.path.join(input_dir, 'entity2id.txt'), 'r', encoding='utf-8') as f:
        ...
        with open(os.path.join(input_dir, 'relation2id.txt'), 'r', encoding='utf-8') as f:
        ...
        self.__train_triple = load_triple(os.path.join(input_dir, 'train.txt'))
        print("N_TRAIN_TRIPLES: %d" % self.__train_triple.shape[0])

        self.__test_triple = load_triple(os.path.join(input_dir, 'test.txt'))
        print("N_TEST_TRIPLES: %d" % self.__test_triple.shape[0])

        self.__valid_triple = load_triple(os.path.join(input_dir, 'valid.txt'))
...
parser = argparse.ArgumentParser(description='ProjE.')
    ...
    model = ProjE(args.input_dir, embed_dim=args.dim, combination_method=args.combination_method,
