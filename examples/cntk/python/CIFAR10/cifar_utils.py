...
def loadData(src, datadir):
    print ('Downloading ' + src)
    fname, h = urlretrieve(src, os.path.join(datadir, 'delete.me'))
    print ('Done.')
    try:
        ...
        for i in range(5):
            batchName = os.path.join(datadir, 'cifar-10-batches-py', 'data_batch_{0}'.format(i + 1))
            trn = np.vstack((trn, readBatch(batchName)))
        print ('Done.')
        print ('Preparing test set...')
        tst = readBatch(os.path.join(datadir, 'cifar-10-batches-py', 'test_batch'))
        print ('Done.')
    finally:
        os.remove(fname)
    return (trn, tst)
...
def saveTrainImages(filename, foldername, datadir):
    ...
    with open(os.path.join(datadir, 'train_map.txt'), 'w') as mapFile:
        with open(os.path.join(datadir, 'train_regrLabels.txt'), 'w') as regrFile:
            for ifile in range(1, 6):
                with open(os.path.join(datadir, 'cifar-10-batches-py', 'data_batch_' + str(ifile)), 'rb') as f:
                    if sys.version_info[0] < 3: 
                        data = cp.load(f)
                    else: 
    ...
    saveMean(os.path.join(datadir, 'CIFAR-10_mean.xml'), dataMean)

def saveTestImages(filename, foldername, datadir):
    if not os.path.exists(foldername):
      os.makedirs(foldername)
    with open(os.path.join(datadir, 'test_map.txt'), 'w') as mapFile:
        with open(os.path.join(datadir, 'test_regrLabels.txt'), 'w') as regrFile:
            with open(os.path.join(datadir, 'cifar-10-batches-py', 'test_batch'), 'rb') as f:
                if sys.version_info[0] < 3: 
                    data = cp.load(f)
                else: 
                    data = cp.load(f, encoding='latin1')
