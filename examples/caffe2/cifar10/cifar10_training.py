...
from caffe2.experiments.python import SparseTransformer
...
def DownloadDataset(url, path):
    ...
    extracted_dir_path = os.path.join(path, 'cifar-10-batches-bin')
    if not os.path.exists(extracted_dir_path):
        tarfile.open(filepath, 'r:gz').extractall(path)

def GenerateDB(data_folder, train_name, test_name):
    train_db_path = os.path.join(data_folder, train_name)
    test_db_path = os.path.join(data_folder, test_name)
    print('DB: ', train_name, ' and ', test_name)
    if not os.path.exists(train_db_path) and not os.path.exists(test_db_path):
        from distutils.sysconfig import get_python_lib
        sitepack = get_python_lib()
        syscall = sitepack + r"\caffe2\binaries\binaries\Release\make_cifar_db --db lmdb --input_folder " + os.path.join(data_folder, 'cifar-10-batches-bin') + \
        " --output_train_db_name " + train_db_path + " --output_test_db_name " + test_db_path
...
def trainNtest(model_gen, args):
    ...
    data, label = AddInput(
        ...
        db=os.path.join(args.input_dir, args.train_input_name),
    ...
    if args.gpu and C.has_gpu_support:
    ...
    workspace.ResetWorkspace(args.output_dir)
    ...
    data, label = AddInput(
        ...
        db=os.path.join(args.input_dir, args.test_input_name),
        db_type=args.db_type)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='cifar-10 Tutorial')
    ...
    parser.add_argument("--train_input_name", type=str,
                        default=None,
                        required=True,
                        help="Path to the database for training data")
    parser.add_argument("--test_input_name", type=str,
                        default=None,
                        required=True,
                        help="Path to the database for test data")
    parser.add_argument("--db_type", type=str,
                        default="lmdb", help="Database type")
    ...
    if not os.path.exists(args.train_input_name):
        DownloadDataset(url, args.input_dir)
    GenerateDB(args.input_dir, args.train_input_name, args.test_input_name)
